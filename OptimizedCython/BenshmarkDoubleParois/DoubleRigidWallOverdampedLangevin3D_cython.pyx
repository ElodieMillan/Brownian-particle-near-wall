...
class RigidWallOverdampedLangevin3D( InertialLangevin3D ):
    def __init__(self, dt, Nt, a, Hp, rho=1050.0, rhoF=1000.0, eta=0.001, T=300.0, x0=None):
        """
        ...
        if x0 == None:
            x0 = (0.0, 0.0, a)
        super().__init__(dt, Nt, a, rho, eta=eta, T=T, x0=x0)
        self.Hp = Hp
        self.rhoF = rhoF
        self.lD = 70e-9  # Debay length
        self.g = 9.81  # m/s²
        self.m = rho * (4 / 3) * np.pi * self.a ** 3
        self.delta_m = (4 / 3) * np.pi * self.a ** 3 * (self.rho - self.rhoF) # m_particule - m_fluid
        self.lB = (self.kb * self.T) / (self.delta_m * self.g)  # Boltzmann length
        self.H = self.Hp - self.a
    ...
    def _gamma_xy(self, zi_1):
        ...
        """
        # Libchaber formula
        xi = self.a / (zi_1+self.a)

        gamma_xy = (6 * np.pi * self.a * self.eta *
    ...
    def trajectory(self, output=False):
        ...
        res = np.asarray(trajectory_cython(self.Nt, rngx, rngy, rngz, res, self.delta_m, self.g, self.dt,
                                self.kb, self.T, self.lD, self.a, self.eta, self.H))
...
cdef double gamma_xy(double zi_1, double a, double eta, double Hs):
    """
    :param zi_1: Particule position at time t-dt.
    :param a: Particule ray.
    ...
    cdef double gam_xy = (
        ...
        * a
        * eta
        # * quelque chose en fonction de H et a !!!!!
        * ( 1 - (zi_1 / Hs)**2 )**(-1)
    )
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef double gamma_z(double zi_1, double a, double eta, double H):
    """
    :param zi_1: Particule position at time t-dt.
    :param a: Particule ray.
    :param eta: Fluide viscosity.
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef double w(double gamma, double kb, double T):
    """
...
cdef double positionXYi_cython(double xi_1, double zi_1, double rng, double dt,
                               double kb, double T, double a, double eta):
    """
    ...
    :param a: aay of the particule.
    :param eta: Fluid viscosity.
    
    :return: X or Y position of the particule at time t.
    """
    cdef double gamma = gamma_xy(zi_1, a, eta)
    cdef double xi = xi_1 +w(gamma, kb, T) * rng * dt
...
cdef double positionZi_cython(double zi_1, double rng, double delta_m, double g, double dt,
                               double kb, double T, double lD, double a, double eta, double H):
    """
    :param xi_1: Position of the particule at (t - dt).
    :param zi_1: Perpendicular position by the wall z at (t - dt).
    :param rng: a random number for dBt/dt white noise.
    ...
    :param T: Temperature.
    :param lD: Debay length.
    :param a: Ray of the particule.
    :param eta: Fluid viscosity.
    
    :return: Z position of the particule at time t.
    """
    cdef double gamma = gamma_z(zi_1, a, eta, H)
    ...
    cdef double correction = ( #spurious drift
                kb * T
                * (42 * a * zi_1 ** 2 + 24 * a ** 2 * zi_1 + 4 * a ** 3)
                / ( (6 * zi_1 ** 2 + 9 * a * zi_1 + 2 * a ** 2) * (6*zi_1**2 + 2*a*zi_1) )
                * dt
                / gamma
    )
    cdef double zi = zi_1 - weight + elec + correction +w(gamma, kb, T) * rng * dt
...
cdef double[:,:] trajectory_cython(unsigned long int Nt,
    ...
    for i in range(1, Nt):
        res[0,i] = positionXYi_cython(res[0,i - 1], res[2,i - 1], rngx[i], dt, kb, T, a, eta)
        res[1,i] = positionXYi_cython(res[1,i - 1], res[2,i - 1], rngy[i], dt, kb, T, a, eta)
        res[2,i] = positionZi_cython(res[2,i - 1], rngz[i], delta_m, g, dt, kb, T, lD, a, eta, H)
