...
-> Pas de force de répultion électronique.
"""
...
class RigidWallOverdampedLangevin3D( InertialLangevin3D ):
    def __init__(self, dt, Nt, a, H, lB, Nt_sub=1, rho=1050.0, rhoF=1000.0, eta=0.001, kBT=1.38e-23*300, x0=None):
    ...
    def P_z_wall(self, z):
        ...
        if type(z) != np.ndarray:
            if (z > self.H) or (z < -self.H):
                return 0
            return np.exp(- z / self.lB)
        Pz = lambda z : np.exp(- z / self.lB)
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef double Forces(double z, double kBT, double lB):
...
cdef double positionZi_cython(double zi_1, double rng, double dt, double a,
    ...
    cdef double zi = zi_1  + Dprime_z(zi_1, kBT, eta, a, H )*dt + Forces(zi_1, kBT, lB)*dt /gamma +  w(gamma, kBT)*rng*dt
    if zi < -(H):
        zi = -2*H - zi
    if zi > H:
        zi =  2*H - zi
...
cdef double[:,:] trajectory_cython(unsigned long int Nt,
    ...
    for i in range(1, Nt):
        for j in range(0, Nt_sub):
            ...
            z = positionZi_cython(z, random_gaussian()/sqrt(dt), dt, a, eta, kBT, H, lB)
